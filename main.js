...
function renderTabs() {
    ...
    if(label) label.innerText = activeTab || 'no file selected';
}
...
function switchTab(tabId, event) {
    ...
    if (tabId === 'editor') {
        ...
        updateProjectStats(); 
    } else if (tabId === 'compare' && typeof initDiffViewer === 'function') {
        initDiffViewer(); // Hook for the upcoming diff.js module
    }
...
function pushToTrash(filename, content) {
    localforage.getItem('devos_trash').then(trash => {
        trash = trash || {};
        trash[filename] = content;
        localforage.setItem('devos_trash', trash).then(renderTrash);
    });
}
...
async function factoryReset() {
    ...
    if (confirmation === 'NUKE') {
        try {
            ...
            await localforage.removeItem('devos_trash');
            await localforage.removeItem('vault_snippets');
            
            localStorage.removeItem('settings_theme');
            localStorage.removeItem('settings_show_toolbar');
