(function () {
    ...
    const DEFAULT_SETTINGS = {
        ...
        followsChunkSize: 3,
        currentMode: 'bananas' // Save the last used mode
    };

    // --- *** NEW: Load State from localStorage *** ---
    // Load settings, merging with defaults to ensure new settings aren't missed
    let settings = Object.assign({}, DEFAULT_SETTINGS, JSON.parse(localStorage.getItem('followMeSettings')));
    
    // Load app state (sequences), or create a new one
    ...
    // Set current mode from loaded settings
    let currentMode = settings.currentMode;
    
    const getCurrentState = () => appState[currentMode];

    // --- *** NEW: Save State Function *** ---
    /**
     * Saves the current settings and app state to localStorage.
     */
    ...
    // --- All other functions (renderSequences, addValue, handleBackspace, etc.)
    //     are defined inside window.onload to ensure DOM is ready ---

    // --- Initialization ---
    window.onload = function() {
        
        // --- DOM Elements (Declared *after* window.onload) ---
        ...
        function renderSequences() {
            ...
            activeSequences.forEach((set, index) => {
                ...
                // Add (set || []) to handle potential undefined sequences during state transitions
        ...
        function addValue(value) {
            ...
            if (!sequences[targetIndex]) {
                sequences[targetIndex] = [];
            }
        ...
        function openSettingsModal() {
            ...
            // Ensure follows state exists before accessing it
            if (!appState['follows']) {
        ...
        function generateHelpContent() {
            // Ensure rounds15 state exists before accessing
            if (!appState['rounds15']) {
        ...
        function handleBananasDemo() {
            ...
            if (!sequenceToPlay || sequenceToPlay.length === 0 || (demoButton && demoButton.disabled)) {
                if (demoButton && demoButton.disabled) return;
            ...
            for (let chunkNum = 0; chunkNum < numChunks; chunkNum++) {
                for (let seqIndex = 0; seqIndex < numSequences; seqIndex++) {
                    for (let k = 0; k < chunkSize; k++) {
                        ...
                        if (activeSequences[seqIndex] && valueIndex < activeSequences[seqIndex].length) {
        ...
        function handleRounds15Demo() {
            ...
            if (!sequenceToPlay || sequenceToPlay.length === 0 || (demoButton.disabled && !settings.isRounds15ClearAfterPlaybackEnabled) ) {
        ...
        function initializeListeners() {
            ...
            // --- *** NEW: Add saveStateToStorage() to listeners *** ---
            ...
            function setupSpeedSlider(slider, displayElement, modeKey) {
                ...
                slider.addEventListener('change', saveStateToStorage); // *** SAVE on release ***
            }
            ...
            uiScaleSlider.addEventListener('change', saveStateToStorage); // *** SAVE on release ***
